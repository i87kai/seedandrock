...
| File | Role |
| --- | --- |
| `SurvivalGraph.cs` | The world design: island shape, spawn meadow, forests, lake, bay, foothills, mountain, texture masks and placement masks/densities. Built from `Spot / Noise / Blend / Levels / Mask / Selector / Slope / UnityCurve / Beach` nodes only. |
...
| Island | centre (800, 800), land ≈ 1.45 km across, 1.5 km² | Small enough to learn; every landmark is visible from the lookout hill. |
...
```
                 N
     bay/beach        mountain (snow cap)
        west forest     foothills
                 spawn ✚   lake   east forest
   lookout hill       plains        cove/beach
                 S
```

Spawn `(620, 560)` is a flat 50 m meadow (r 110) with a small grove 140 m north-west, a stone
outcrop 100 m east, the lake 360 m east, the lookout hill 260 m south-west and the mountain on the
horizon 800 m north-east.
